...
$('div.address-feedback').on("mousedown", "a", function(event) {
    ...
    $('.address_auto_filled').val($(this).attr("data-name"));
    $('.postalCode_auto_filled').val($(this).attr("data-postcode"));
    $('.city_auto_filled').val($(this).attr("data-city"));

    $('.address-feedback').empty();
});

// On espionne le clavier dans le champ adresse pour déclencher les actions qui vont bien
$('.address_auto_filled').keyup(function(event) {
    // Stop la propagation par défaut
    event.preventDefault();
    event.stopPropagation();
    ...
    let rue = $('.address_auto_filled').val();
    if (rue.length === 0) {
        $('.address-feedback').empty();
        return false;
    }
